...
(() => {
  ...
  let player = {
      ...
      //点击播放，暂停
      clickPlayPause(){
        let play_pause = $(this.play_pause),
            music = this.music;
      ...
      //更新总时长
      updateDuration(){
        let duration = Math.ceil(this.music.duration),
            durationStr = this.timeToString(duration);
        if(duration){
      ...
      //点击进度条切换进度
      progressControl(){
        let elem = this.progressContainer;
      ...
      updateVolume(){
        ...
        let percent = Math.floor(this.volume * 100) + "%";
        $(this.volumeDiv).setStyle("width", percent);
      },
      setVolume(){
        this.updateVolume();
        let elem = this.volumeContainer;
          ...
          $f.watch(player, 'ListIndex', () => {
            if(this.ListSongs){
              ...
              for(let i = 0;i<$i.length;i++){
               ...
               //防止越界（ListIndex在更新最后一首歌曲时先越界再回零然后切换歌曲，因此此处有越界）
              let index = this.ListIndex >= this.ListLength?0:this.ListIndex;
